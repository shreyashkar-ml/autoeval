...
type ManagedKey = "params" | "report_guidance";
...
const runLabel = (value: string) => value.split("_").pop() || short(value);
const fmtDuration = (value?: number | null) => value == null ? "none" : value < 1000 ? `${value} ms` : `${(value / 1000).toFixed(1)} s`;
const fmtBytes = (value = 0) => value < 1024 ? `${value} B` : value < 1024 ** 2 ? `${(value / 1024).toFixed(1)} KB` : `${(value / 1024 ** 2).toFixed(1)} MB`;
const fmtTime = (value?: string | null) => value ? value.replace("T", " ").replace(/Z$/, "") : "none";
...
function drag(event: ReactPointerEvent, move: (clientX: number) => void) {
  event.preventDefault();
  const update = (next: PointerEvent) => move(next.clientX);
  const stop = () => {
...
function App() {
  ...
  const [railWidth, setRailWidth] = useState(() => Number(localStorage.getItem("autoexp-rail-width")) || 270);
  ...
  useEffect(() => {
    ...
    api<{ session: ReviewSession }>(`/api/review?token=${encodeURIComponent(reviewToken)}`)
      .then(value => {
        setExperimentId(value.session.experiment_id);
  ...
  useEffect(() => {
    const open = () => { setPrimaryView("overview"); setSelection({ kind: "managed", key: "report_guidance" }); setView("bench"); };
    window.addEventListener("autoexp-report-guidance", open);
    return () => window.removeEventListener("autoexp-report-guidance", open);
  }, []);
  useEffect(() => {
    if (!toast) return;
  ...
  const statusText = activeRun ? `${activeRun.status} ${runLabel(activeRun.run_id)} / ${basename(activeRun.script_name)}` : data?.research?.loop.active ? `${data.research.loop.phase} / ${data.research.loop.status}` : `idle / ${data?.runs.length || current?.run_count || 0} runs`;
  ...
  const resizeRail = (clientX: number) => {
    const width = Math.min(480, Math.max(190, clientX));
    setRailWidth(width);
    localStorage.setItem("autoexp-rail-width", String(width));
  };
  const resizeInspector = (clientX: number) => {
    const work = document.querySelector<HTMLElement>(".pane-work");
    if (!work) return;
    const box = work.getBoundingClientRect();
    setInspectorWidth(Math.min(80, Math.max(20, 100 - (clientX - box.left) / box.width * 100)));
  };

  return <div className="app">
    <header>
      <button className="wordmark" disabled={Boolean(reviewToken)} onClick={() => { setPrimaryView("overview"); setSelection({ kind: "registry" }); setView("bench"); }} title={reviewToken ? "Review is locked to its experimentation" : "All experimentations"}><b>auto<i>exp</i></b><span className="tag">local autonomous experimentation</span></button>
      ...
      <label className="proj-sel">experiment / <select value={experimentId || ""} disabled={Boolean(reviewToken)} onChange={event => chooseExperiment(event.target.value)} aria-label="Experimentation">
      ...
      <div className="hd-right">
        ...
        <div className="view-toggle" role="tablist" aria-label="View"><button aria-pressed={view === "bench"} onClick={() => setView("bench")}>Experiment</button><button id="report-toggle" aria-pressed={view === "report"} onClick={() => setView("report")}>Overall report</button></div>
    ...
    <div className={railOpen ? "body" : "body rail-collapsed"} style={{ "--rail-width": `${railWidth}px` } as CSSProperties}>
      <Rail data={data} selection={selection} primaryView={primaryView} filter={filter} onFilter={setFilter} onSelect={navigate} />
      <div className="rail-resizer" role="separator" aria-label="Resize sidebar" aria-orientation="vertical" aria-valuemin={190} aria-valuemax={480} aria-valuenow={railWidth} tabIndex={railOpen ? 0 : -1} onPointerDown={event => drag(event, resizeRail)} onKeyDown={event => {
        if (event.key === "ArrowLeft" || event.key === "ArrowRight") resizeRail(railWidth + (event.key === "ArrowLeft" ? -16 : 16));
      }} />
      <main className="pane">
        <div className={`pane-work${inspectorOpen ? " split" : ""}`} style={{ "--inspector-width": `${inspectorWidth}%` } as CSSProperties}>
          ...
          {inspectorOpen ? <div className="pane-resizer" role="separator" aria-label="Resize inspector" aria-orientation="vertical" aria-valuemin={20} aria-valuemax={80} aria-valuenow={Math.round(inspectorWidth)} tabIndex={0} onPointerDown={event => drag(event, resizeInspector)} onKeyDown={event => {
            if (event.key === "ArrowLeft" || event.key === "ArrowRight") setInspectorWidth(width => Math.min(80, Math.max(20, width + (event.key === "ArrowLeft" ? 2 : -2))));
          }} /> : null}
          {inspectorOpen && data ? <aside className="inspector" aria-label={`${scopeLabel(selection, view, data)} inspector`}>
            <div className="inspector-head">
              <span>{scopeLabel(selection, view, data)}</span>
        ...
        <ReviewDock token={reviewToken} selection={selection} view={view} experimentId={experimentId} data={data} onSelect={navigate} onView={setView} onToast={setToast} />
      </main>
    </div>
...
function Rail({ data, selection, primaryView, filter, onFilter, onSelect }: { data: ExperimentPayload | null; selection: Selection; primaryView: PrimaryView; filter: string; onFilter: (value: string) => void; onSelect: (value: Selection) => void }) {
  ...
  if (!data) return <nav id="experiment-rail" className="rail" aria-label="Experimentation"><div className="filter"><input id="filterBox" aria-label="Filter experimentation" placeholder="filter…" value={filter} onChange={event => onFilter(event.target.value)} /></div></nav>;
  ...
  return <nav id="experiment-rail" className="rail" aria-label="Experimentation">
    <div className="filter"><input id="filterBox" aria-label="Filter experimentation" placeholder="filter…" value={filter} onChange={event => onFilter(event.target.value)} /></div>
    <RailRow active={primaryView === "overview"} onClick={() => onSelect({ kind: "overview" })} name="Overview" />
    {workspaceGroups.map(group => { const files = data.files.filter(file => group.roles.includes(file.role) && hit(file.path)); const params = group.label === "Inputs" && hasParams && hit("parameters"); const guidance = group.label === "Inputs" && hit("report instructions"); return files.length || params || guidance ? <div className="rail-group" key={group.label}>
      ...
      {params ? <RailRow active={selection.kind === "managed" && selection.key === "params"} onClick={() => onSelect({ kind: "managed", key: "params" })} name="Parameters" meta="input" /> : null}
      {guidance ? <RailRow active={selection.kind === "managed" && selection.key === "report_guidance"} onClick={() => onSelect({ kind: "managed", key: "report_guidance" })} name="report-instructions.md" mono meta="guidance" /> : null}
    </div> : null; })}
    {data.research ? <>
      <div className="rl-sec">Research</div>
      <RailRow active={primaryView === "research"} onClick={() => onSelect({ kind: "research" })} name="Research" meta={`best ${data.research.objective.best ?? "none"}`} />
      {[...data.research.experiments].sort((a, b) => a.sequence - b.sequence).filter(item => hit(`${item.attempt_id} ${item.hypothesis}`)).map(item => <RailRow key={item.key} active={selection.kind === "run" && selection.id === item.run_id} onClick={() => item.run_id ? onSelect({ kind: "run", id: item.run_id, tab: "diff" }) : onSelect({ kind: "research" })} name={item.attempt_id} mono indent status={item.verdict || item.status} score={item.score == null ? "none" : String(item.score)} title={item.hypothesis} />)}
    </> : <>
...
function Registry({ repositories, onSelect }: { repositories: Repository[]; onSelect: (id: string) => void }) {
  ...
  return <><h1 className="title">Experimentations</h1><p className="lede">Every agent experiment across every repository, indexed centrally and served from one local UI.</p><div className="kv"><span>registry <b>global Autoexp data</b></span><span>server <b>localhost / autoexp view</b></span></div>
...
function Overview({ data, onSelect }: { data: ExperimentPayload; onSelect: (value: Selection) => void }) {
  ...
  return <div className="primary-view"><div className="primary-fixed"><h1 className="title">{data.experiment.title}</h1><p className="lede">{data.experiment.objective}</p><div className="kv"><span>repo <b>{data.experiment.repo_path}</b></span><span>kind <b>{data.experiment.kind}</b></span><span>runner <b>{data.experiment.runner}</b></span><span>experiment <b>{data.experiment.experiment_id}</b></span></div>
    ...
    <section><div className="lbl">Overall report</div>{data.project_report.exists ? <><p className="lede report-excerpt">{reportHeading ? <><strong>{reportHeading}</strong>{" "}</> : null}{reportExcerpt}…</p><p><button className="link" onClick={() => window.dispatchEvent(new CustomEvent("autoexp-report"))}>Open the overall report →</button>{reportDocument ? <> <span className="sep">·</span> <a className="link" href={`/api/experiments/${data.experiment.experiment_id}/documents?path=${encodeURIComponent(reportDocument.path)}&download=1`}>Download report.md</a></> : null}</p></> : <p className="empty-copy">No overall report yet. Per-experiment milestone reports remain available on their runs.</p>}</section>
    </div></div>;
}
...
function RunTable({ runs, onRun }: { runs: Run[]; onRun: (id: string, tab?: RunTab) => void }) {
  return runs.length ? <div className="table-scroll"><table className="t run-table"><thead><tr><th>run</th><th>title</th><th>status</th><th>inspect</th><th className="num">when</th></tr></thead><tbody>{runs.map(run => <tr className="rowlink" key={run.run_id} onClick={() => onRun(run.run_id, "source")}><td className="mono" title={run.run_id}>{runLabel(run.run_id)}{run.parent_run_id ? <span className="faint"> ↳ {runLabel(run.parent_run_id)}</span> : null}</td><td className="serif">{run.title || basename(run.script_name)}</td><td><Status value={run.status} /></td><td><div className="evidence-links"><button onClick={event => { event.stopPropagation(); onRun(run.run_id, "source"); }}>source</button><button onClick={event => { event.stopPropagation(); onRun(run.run_id, "output"); }}>artifacts</button><button onClick={event => { event.stopPropagation(); onRun(run.run_id, "logs"); }}>logs</button>{run.report_path ? <button onClick={event => { event.stopPropagation(); onRun(run.run_id, "report"); }}>report</button> : null}<button onClick={event => { event.stopPropagation(); onRun(run.run_id, "diff"); }}>diff</button></div></td><td className="num">{fmtTime(run.created_at)}</td></tr>)}</tbody></table></div> : <p className="empty-copy">No runs yet. The first execution will seal the declared inputs.</p>;
}
...
function Milestone({ item, onRun }: { item: ExperimentPayload["milestones"][number]; onRun: (id: string) => void }) {
  ...
  return <div className="mile"><span className={`tag ${tag}`}>{tag}</span><span className="mile-copy"><strong>{item.title}</strong><span className="txt">{item.significance}</span></span>{item.target_kind === "run" ? <button className="ref link" onClick={() => onRun(item.target_id)}>{runLabel(item.target_id)}</button> : <span className="ref">{item.target_id}</span>}</div>;
}

function managedPath(key: ManagedKey) { return key === "report_guidance" ? "report-instructions.md" : "parameters.json"; }
function ManagedView({ data, managedKey }: { data: ExperimentPayload; managedKey: ManagedKey }) {
  ...
  return managedKey === "report_guidance" ? <ReportGuidanceEditor experimentId={data.experiment.experiment_id} text={text} /> : <><h1 className="title">Run parameters</h1><div className="kv"><span><b>input</b></span><span>values used by every execution</span></div><section className="viewer-section"><CodeViewer path={managedPath(managedKey)} text={text} right="current values" sealed="read-only" /></section></>;
}

function ReportGuidanceEditor({ experimentId, text }: { experimentId: string; text: string }) {
  const [draft, setDraft] = useState(text);
  const [saved, setSaved] = useState(text);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => { setDraft(text); setSaved(text); }, [experimentId, text]);
  const save = () => {
    setSaving(true); setError("");
    api<{ text: string }>(`/api/experiments/${experimentId}/report-guidance`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: draft }) })
      .then(value => { setDraft(value.text); setSaved(value.text); })
      .catch(reason => setError(String(reason)))
      .finally(() => setSaving(false));
  };
  return <><h1 className="title">Report generation instructions</h1><div className="kv"><span><b>guidance</b></span><span>used for future per-experiment and overall reports</span><span>existing reports remain immutable</span></div><section className="viewer-section guidance-editor"><div className="ed"><div className="ed-h"><span>report-instructions.md</span><span className="grow" /><span className="ed-note">editable global instruction</span></div><textarea className="src-edit" aria-label="Report generation instructions" value={draft} onChange={event => setDraft(event.target.value)} spellCheck={false} /></div><div className="guidance-actions"><span className={error ? "failed" : ""}>{error || (draft === saved ? "Saved" : "Unsaved changes")}</span><button className="dbtn primary" disabled={saving || draft === saved} onClick={save}>{saving ? "Saving…" : "Save instructions"}</button></div></section></>;
}
...
function ScoreChart({ data }: { data: ExperimentPayload }) {
  ...
  const width = 720, height = 150, left = 52, right = 16, top = 12, bottom = 30;
  ...
  const yTicks = Array.from({ length: 3 }, (_, index) => low + (high - low) * index / 2);
  ...
  const tooltipTop = hovered ? Math.min(Math.max(y(Number(hovered.score)) - 48, top + 4), height - bottom - 46) : 0;
...
function RunSourcePane({ run }: { run: Run }) {
  ...
  return <div className="artifact-browser source-browser"><div className="artifact-files"><div className="artifact-label">Snapshot source / {source.files.length}</div>{source.files.map(item => <button className={`frow${item.path === file.path ? " selected" : ""}`} key={item.path} title={item.path} onClick={() => setSelected(item.path)}><span>{basename(item.path)}</span><span className="fnote">{item.path}</span></button>)}</div><div className="artifact-detail"><div className="codehead"><span>{file.path}</span><span className="right">sealed at {short(run.source_snapshot_id)} / immutable evidence</span></div><CodeViewer path={`runs/${run.run_id}/source/${file.path}`} text={file.text} /></div></div>;
}
...
function RunReportPane({ run, runs, onRun }: { run: Run; runs: Run[]; onRun: (id: string) => void }) {
  const [report, setReport] = useState<{ path: string; text: string; artifact?: Artifact | null; document?: Document | null } | null>(null);
  useEffect(() => { setReport(null); api<{ path: string; text: string; artifact?: Artifact | null; document?: Document | null }>(`/api/runs/${run.run_id}/report`).then(setReport); }, [run.run_id]);
  if (!report) return <Loading />;
  return report.text ? <div className="doc"><div className="sub">{report.document ? "Per-experiment report" : "Run report"} / {report.path} / <a className="link" href={`/api/runs/${run.run_id}/report?download=1`}>download</a></div><ReportGuidanceLink /><MarkdownDoc text={report.text} runs={runs} onRun={onRun} /></div> : <p className="empty-copy">No per-experiment report was generated because this run has no important milestone.</p>;
}
...
function DeltaPane({ run }: { run: Run }) {
  ...
  const baseRunId = delta.base_run_id || run.parent_run_id;
  return delta.available ? <div className="diff-shell"><div className="codehead"><span>source diff</span><span className="right">{baseRunId ? `change vs ${runLabel(baseRunId)}` : "change from prior snapshot"}</span></div>{delta.changed_files?.length ? <div className="diff-summary">{delta.changed_files.map(file => <span key={file}>{file}</span>)}</div> : null}<UnifiedDiff text={delta.diff} /></div> : <p className="empty-copy">No earlier source snapshot is available for comparison.</p>;
}
...
function ProjectReport({ data, onRun }: { data: ExperimentPayload; onRun: (id: string) => void }) {
  ...
  return <><h1 className="title">Overall report</h1>{data.project_report.exists ? <><div className="kv"><span>{data.project_report.path}</span>{document ? <a className="link" href={`/api/experiments/${data.experiment.experiment_id}/documents?path=${encodeURIComponent(document.path)}&download=1`}>download report.md</a> : null}</div><ReportGuidanceLink /><section className="detail-scroll"><div className="doc"><MarkdownDoc text={data.project_report.text} runs={data.runs} onRun={onRun} /></div></section></> : <p className="empty-copy report-empty">Nothing here yet. The overall report is separate from per-experiment milestone reports and summarizes the complete experimentation.</p>}</>;
}

function ReportGuidanceLink() {
  return <p className="report-guidance-note">Generation guidance: <button className="link mono" onClick={() => window.dispatchEvent(new CustomEvent("autoexp-report-guidance"))}>report-instructions.md</button> / edits apply to future reports</p>;
}
...
function ReviewDock({ token, selection, view, experimentId, data, onSelect, onView, onToast }: { token: string | null; selection: Selection; view: View; experimentId: string | null; data: ExperimentPayload | null; onSelect: (selection: Selection) => void; onView: (view: View) => void; onToast: (message: string) => void }) {
  const [session, setSession] = useState<ReviewSession | null>(null);
  const [notes, setNotes] = useState<DraftNote[]>([]);
  const [draft, setDraft] = useState("");
  const [open, setOpen] = useState(false);
  const [closingIn, setClosingIn] = useState<number | null>(null);
  const scope = scopeLabel(selection, view, data);
  useEffect(() => {
    if (!token) return;
    let active = true;
  ...
  useEffect(() => {
    ...
    if (closingIn === 0) {
      window.close();
      const timer = window.setTimeout(() => setClosingIn(-1), 250);
      return () => window.clearTimeout(timer);
    }
    if (closingIn < 0) return;
    const timer = window.setTimeout(() => setClosingIn(closingIn - 1), 1000);
    return () => window.clearTimeout(timer);
  }, [closingIn]);
  ...
  if (closingIn !== null) return <div id="dock"><div className="dock"><div className="row"><span className="msg">{`${session?.decision === "dismissed" ? "Review closed" : session?.decision === "approved" ? "Approval sent" : "Feedback sent"} / ${closingIn < 0 ? "automatic close was blocked; you can close this tab" : `closing in ${closingIn || "…"}`}`}</span></div></div></div>;
  ...
  const submit = (decision: "approved" | "feedback" | "dismissed") => {
    if (decision === "feedback" && !pending.length) return;
    const batch = decision === "dismissed" ? [] : pending;
    api<{ session: ReviewSession }>("/api/review/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, decision, notes: batch.map(note => ({ scope: note.scope, text: note.text })) }) }).then(value => { setSession(value.session); setNotes([]); setDraft(""); setOpen(false); setClosingIn(3); onToast(decision === "approved" ? "Approved / agent resumed" : decision === "dismissed" ? "Review closed without feedback" : "Follow-up sent / agent resumed with your notes"); });
  };
  return <div id="dock"><div className="dock"><div className="row"><span className="pulse" /><span className="who">agent</span><span className="msg">Agent is waiting for your review. Inspect the evidence and return one scoped batch.</span><span className="sp" /><span className="fine">{scope}</span></div>{notes.length ? <div className="row">{notes.map((note, index) => <span className="note-chip" key={`${note.scope}-${index}`}><button className="scope" onClick={() => { onView(note.view); onSelect(note.selection); }}>{note.scope}</button><span className="txt">{note.text}</span><button className="x" onClick={() => setNotes(notes.filter((_, item) => item !== index))}>×</button></span>)}</div> : null}{open ? <><textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); add(); } }} placeholder={`Note on ${scope} - e.g. compare this with the failed parent before deciding.`} /><div className="row"><span className="fine">Enter to add / Shift+Enter for a line break</span><span className="sp" /><button className="dbtn" onClick={() => { setOpen(false); setDraft(""); }}>Cancel</button><button className="dbtn primary" disabled={!draft.trim()} onClick={add}>Add note</button></div></> : <div className="row"><button className="dbtn" onClick={() => setOpen(true)}>Send note to agent for “{scope}”</button><button className="dbtn" onClick={() => submit("dismissed")}>Close without feedback</button><span className="sp" /><button className="dbtn" onClick={() => submit("approved")}>Approve{notes.length ? ` with ${notes.length} advisory note${notes.length === 1 ? "" : "s"}` : ""}</button><button className="dbtn primary" disabled={!notes.length} onClick={() => submit("feedback")}>Send follow-up ({notes.length})</button></div>}</div></div>;
}

function scopeLabel(selection: Selection, view: View, data: ExperimentPayload | null) {
  if (view === "report") return "overall report";
  ...
  if (selection.kind === "run") {
    const run = data?.runs.find(item => item.run_id === selection.id);
    return runScopeLabel(run, selection.id);
  }
  if (selection.kind === "document") return "document";
  if (selection.kind === "research") return data?.experiment.title || "research";
  return "overview";
}

function runScopeLabel(run: { title?: string | null; script_name: string } | undefined, id: string) {
  return (run?.title || (run ? basename(run.script_name) : short(id))).slice(0, 100);
}
if (import.meta.env.DEV) console.assert(runScopeLabel({ title: "x".repeat(101), script_name: "run.py" }, "id").length === 100, "Review run title scope");
